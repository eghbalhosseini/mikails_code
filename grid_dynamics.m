function [spikes] = gc_dynamics(index,filename,dt, useSpiking,simulate_non_periodic)
%---------------------------------------------------------------
% GC Dynamics 
%---------------------------------------------------------------
%
% This code is in reference to Burak Y, Fiete I (2009) 
...
alpha  = 1.3; % The velocity gain from Equation (4)

%---------------
% RUN SIMULATION many times
%---------------
for runs = index:index+4


    if simulate_non_periodic == 0
           [spikes] = grid_periodic(runs,filename,n,tau,dt,beta,alphabar,abar,wtphase,alpha, useSpiking);
    else
           [spikes] = gc_non_periodic(runs,filename,n,tau,dt,beta,alphabar,abar,wtphase,alpha, useSpiking);
    end
end
